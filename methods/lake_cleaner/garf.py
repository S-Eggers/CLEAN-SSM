...
class GARFRunner():
    ...
    def run(self):
        ...
        # heuristic 4: discard all rules with a length > 2
        rule = eval(open(os.path.join(self.base_dir, "data", "save", "rules_final.txt"), "r").read())
        rules_forward = {k: v for k, v in rule.items() if len(eval(k)) <= 2}
        with open(os.path.join(self.lake_dataset_path, "rules_forward.txt"), "w") as f:
        ...
        rule = eval(open(os.path.join(self.base_dir, "data", "save", "rules_final.txt"), "r").read())        
        rules_forward = {k: v for k, v in rule.items() if len(eval(k)) <= 2}
        with open(os.path.join(self.lake_dataset_path, "rules_backward.txt"), "w") as f:
            f.write(str(rules_forward))

from methods.distilled_garf.SeqGAN.models import GeneratorPretraining, Generator
from methods.distilled_garf.SeqGAN.utils import GeneratorPretrainingGenerator
from methods.distilled_garf.SeqGAN.distiller import DistilledGenerator, GeneratorDistilledGenerator
import os
import keras 
...
class GeneratorTrainer(object):
    ...
    def __init__(self, order, B, T, g_E, g_H, d_E, d_H, d_dropout, generate_samples,path_pos, path_neg, path_rules, g_lr=1e-3, d_lr=1e-3, n_sample=16,  init_eps=0.1, remove_amount_of_error_tuples=0):
        ...
        self.dis_data = GeneratorDistilledGenerator(2.0, self.path_pos, order=order, B=B, T=T, min_count=1, remove_amount_of_error_tuples=remove_amount_of_error_tuples)
        ...
        self.generator_dis = DistilledGenerator(self.V, g_E, g_H)         #A 3-layer neural network input-embedding-lstm-dense

        self.generator = Generator(sess, B, self.V, g_E, g_H // 2, g_lr)
        
        self.rule={}
    
        
    def distill(self, g_epochs=3, g_dis_path=None, lr=1e-3):
        print("Knowledge distillation")
        if g_dis_path is None:
            self.g_dis_path = os.path.join(self.top, 'data', 'save', 'generator_dis.hdf5') 
        else:
            self.g_dis_path = g_dis_path
        
        g_adam = keras.optimizers.Adam(0.01)
        self.generator_dis.compile(g_adam, loss=[keras.losses.categorical_crossentropy, keras.losses.kullback_leibler_divergence])
        
        print('Generator distillation')
        self.generator_dis.summary()
        # print("++++++++++++++++++++")
        self.dis_data.set_teacher_model(self.g_pre_path, self.V, self.g_E, self.g_H)
        self.generator_dis.fit_generator(
            self.dis_data,
            epochs=40,
            steps_per_epoch=None,
            callbacks=[
                keras.callbacks.LearningRateScheduler(lambda epoch: 0.01 if epoch < 10 else 0.001 if epoch < 20 else 0.0001 if epoch < 30 else 0.00001),
                keras.callbacks.EarlyStopping(monitor="loss", patience=5, min_delta=0.01)
            ]
        )
        self.generator_dis.save_weights(self.g_dis_path)
        self.reflect_dis_train()

    def reflect_dis_train(self):                        #Map layer weights to agent
        self.generator_dis.layers.pop()
        # Re-compile the model after removing the layer
        self.generator_dis.compile("adam", 'categorical_crossentropy')
        i = 0
        for layer in self.generator_dis.layers:

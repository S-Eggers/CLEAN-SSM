...
class Generator():
    ...
    def sampling_word(self, prob):
        ...
        for i in range(self.B):
            p = prob[i]
            p = np.nan_to_num(p) # p is an array of 1-dimensional V columns
            # print("p:",p)
            p_sum = p.sum()
            if p_sum == 0:
                print("All probabilities were zero.")
                action[i] = np.random.choice(self.V)
            else:
                p /= p_sum                                         # Total probability normalization
    ...
    def train_rules(self,rule_len,path):
        ...
        for idx in progress_bar:
            ...
            #if len(words) - 2 > len(label2att) - 1:
            #    print(1, sentence, len(sentence))
            #   print(2, label2att, words, len(words), "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n")

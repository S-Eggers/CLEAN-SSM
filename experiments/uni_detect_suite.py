...
class UniDetectSuite(Suite):
    ...
    def _aggregated_combined_plots(self):
        ...
        self._plot_combined_avg_metric(self.results, "recall", f"Recall (UniDetect)", "Error Rate", "Recall", x_ticks, palette, hue="dataset")
        self._save_plot(os.path.join(self.base_dir, f"recall_combined.png"))
        self._plot_combined_avg_metric(self.results, "precision", f"Precision (UniDetect)", "Error Rate", "Precision", x_ticks, palette, hue="dataset")
        self._save_plot(os.path.join(self.base_dir, f"precision_combined.png"))
        self._plot_combined_avg_metric(self.results, "f1", f"F1 (UniDetect)", "Error Rate", "F1", x_ticks, palette, hue="dataset")
        self._save_plot(os.path.join(self.base_dir, f"f1_combined.png"))
        self._plot_combined_avg_runtime(self.results, "runtime", f"Runtime (UniDetect)", "Error Rate", "Runtime (in Seconds)", x_ticks, palette, hue="dataset")

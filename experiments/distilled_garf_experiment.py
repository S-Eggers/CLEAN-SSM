...
from methods.distilled_garf.main import main as distilled_garf
...
class DistilledGARFExperiment(Experiment):
    def __init__(self, error_rate: float, method_name: str, dataset: str, error_generator: int = 1):
        ...
        self.base_dir = os.path.join(os.getcwd(), "methods", "distilled_garf")
    ...
    def worker(kwargs: Dict[str, Any] = dict()):
        ...
        os.chdir(os.path.join(os.getcwd(), "methods", "distilled_garf"))
        distilled_garf(flag, order, remove_amount_of_error_tuples, dataset)

...
from .rnn_garf_experiment import RNNGARFExperiment
from insert_error import DETECTION


class RNNGARFDetectorSuite(Suite):
    def __init__(self, dataset_name: str, error_generator: int = DETECTION):
        super().__init__("RNN-GARF", "error_detection", dataset_name, error_generator)
    ...
    def _aggregated_combined_plots(self):
        ...
        self._plot_combined_avg_metric(self.results, "recall", f"Recall (RNN-GARF)", "Error Rate", "Recall", x_ticks, palette, hue="dataset")
        self._save_plot(os.path.join(self.base_dir, f"recall_combined.png"))
        self._plot_combined_avg_metric(self.results, "precision", f"Precision (RNN-GARF)", "Error Rate", "Precision", x_ticks, palette, hue="dataset")
        self._save_plot(os.path.join(self.base_dir, f"precision_combined.png"))
        self._plot_combined_avg_metric(self.results, "f1", f"F1 (RNN-GARF)", "Error Rate", "F1", x_ticks, palette, hue="dataset")
        self._save_plot(os.path.join(self.base_dir, f"f1_combined.png"))
        self._plot_combined_avg_runtime(self.results, "runtime", f"Runtime (RNN-GARF)", "Error Rate", "Runtime (in Seconds)", x_ticks, palette, hue="dataset")

...
from methods.lake_cleaner.main import main as lake_cleaner


class LakeCleanerExperiment(Experiment):
    def __init__(self, error_rate: float, method_name: str, dataset: str, error_generator: int = 1):
        ...
        self.base_dir = os.path.join(os.getcwd(), "methods", "lake_cleaner")
        self.lake_dataset_path = os.path.join(self.base_dir, "datasets", "artificial_lake")

    def run(self, **kwargs):
        results = lake_cleaner(self.base_dir, self.lake_dataset_path)

...
class GARFExperiment(Experiment):
    ...
    def run(self, **kwargs):
        ...
        process.start()
        process.join()
        pretraining1, adversarial1, rule1, rule_filter1, repair1, v, lstm_weights = queue.get()
        
        queue = Queue()
        process_kwargs["queue"] = queue
        ...
        process.start()
        process.join()
        pretraining2, adversarial2, rule2, rule_filter2, repair2, _, _ = queue.get()
        end_time = time.time()
        ...
        self.pretraining = pretraining1 + pretraining2
        self.adversarial = adversarial1 + adversarial2
        self.rule = rule1 + rule2
        self.rule_filter = rule_filter1 + rule_filter2
        self.repair = repair1 + repair2
        self.v = v
    ...
    def worker(kwargs: Dict[str, Any] = dict()):
        ...
        pretraining, adversarial, rule, rule_filter, repair, v, lstm_weights = garf_original(flag, order, remove_amount_of_error_tuples, dataset, g_h, g_e)
        queue.put((pretraining, adversarial, rule, rule_filter, repair, v, lstm_weights))
        
    def result(self) -> Tuple[float, float, float, float, float, float, float, float, float, int]:
        return self.precision, self.recall, self.f1, self.runtime, self.pretraining, self.adversarial, self.rule, self.rule_filter, self.repair, self.v, self.lstm_weights

...
from methods.transfer_learning_garf.main import main as transfer_learning_garf
from .experiment import Experiment


class TransferLearningGARFExperiment(Experiment):
    def __init__(self, error_rate: float, method_name: str, dataset: str, error_generator: int = 1):
        ...
        self.base_dir = os.path.join(os.getcwd(), "methods", "transfer_learning_garf")
    ...
    def run(self, **kwargs):
        ...
        process.start()
        process.join()
        process = Process(target=self.worker, args=({"flag": 3, "order": 0, "dataset": self.dataset},))
        process.start()
        process.join()
        end_time = time.time()
    ...
    def worker(kwargs: Dict[str, Any] = dict()):
        ...
        os.chdir(os.path.join(os.getcwd(), "methods", "transfer_learning_garf"))
        transfer_learning_garf(flag, order, remove_amount_of_error_tuples, dataset)

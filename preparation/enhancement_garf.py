...
class GARFEnhancementPreparation(Preparation):
    def __init__(self, 
                 directory: str, 
                 dataset: str, 
                 error_generator: int, 
    ...
    def _prepare(dataset: str = "Hospital",
        ...
        # readd label column
        dataframe["Label"] = ""
        dataset_url = os.path.join(os.path.dirname(os.getcwd()), f"{dataset}_enhancement.csv")        
        dataframe.to_csv(dataset_url, index=False)
        # create database
        base_dir = os.path.join(os.getcwd(), "methods", "garf_original")
        create_database_original(
                base_dir=base_dir, 
                datasets={f"{dataset}_enhancement": [dataset_url]}, 
